...
std::string MinerManager::getFounderAddress(int founderIndex)
{
    if(founderIndex==1) { return MinerManager::vFoundersRewardAddressFone[rand() % 8]; }
	else if (founderIndex == 2) { return MinerManager::vFoundersRewardAddressFtwo[rand() % 8]; }
    else if(founderIndex==3) { return MinerManager::vFoundersRewardAddressFthree[rand() % 8]; }
    else { return ""; }
}
...
BlockMiningParameters MinerManager::requestMiningParameters(System::Dispatcher& dispatcher, const std::string& daemonHost, uint16_t daemonPort, const std::string& miningAddress)
{
    ...
    while (true)
    {
        try
        {
            ...
            if (m_blockCounter % 20 == 0) {
                std::cout << SuccessMsg("Entering mining donation phase.");
                if(founderAdresseId > 3) { founderAdresseId = 1; } else { founderAdresseId++; }
                request.wallet_address = getFounderAddress(founderAdresseId);
            } else {
                std::cout << SuccessMsg("Mining donation phase complete. Thank you for supporting the developers!");
